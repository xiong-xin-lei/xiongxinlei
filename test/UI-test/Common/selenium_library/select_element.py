...
class SelectElement(Base):
    ...
    def get_items_text(self, select_ul, key):
        # select_ul--table表格元素，精确至tbody
        # key--想要查找的值 从0开始
        # i--返回查询值所在li，从1开始
        arr = []
        select_li_list = self.driver.find_element(*select_ul).find_elements_by_tag_name('li')
        for li in select_li_list:
            time.sleep(0.1)
            arr.append(li.text)
