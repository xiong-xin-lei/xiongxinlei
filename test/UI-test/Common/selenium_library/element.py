...
class Element(Base):
    ...
    def get_page_html(self, locator):
        pageHtml = self.find_element(locator).get_attribute("outerHTML")
        return pageHtml
    ...
    def is_button_disabled(self, locator):
        """返回按钮的禁用状态"""
        self.logger.info('按钮禁用')
        return self.get_element_attribute(locator, 'disabled')
    ...
    def mouse_over_center(self, locator):
        """鼠标悬浮在元素上"""
        self.logger.info(f"鼠标悬浮在元素上：{locator}")
        element = self.find_element(locator)
        size = element.size
        ...
        action = ActionChains(self.driver)
        action.move_to_element_with_offset(element, offsetx, offsety)
        action.perform()
