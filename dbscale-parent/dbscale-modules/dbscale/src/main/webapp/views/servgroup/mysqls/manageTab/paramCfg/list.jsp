    ...
    <div class="iframeBorder">
        ...
        <div class="btnParentDiv">
            <div id="btnPanel">
				...
				<el-select v-model="unitValue" placeholder="请选择单元" size="small" @change="unitChange" v-if="highAvailable">
		...
		<vxe-table size="small" :max-height="table_heighth" highlight-hover-row resizable auto-resize border="inner" style="color: #333333;"
				   ...
				   :checkbox-config="{highlight: true, trigger: 'row'}" @sort-change="sortChange"
				   :sort-config="{trigger: 'cell',remote : 'true'}" keep-source
				   :edit-config="{trigger: 'dblclick', mode: 'row', showStatus: true,activeMethod: activeCellMethod, icon: editIcon()}"
				   :edit-rules="validRules" @edit-closed="editClosedEvent" :row-class-name="tableRowClassName">
			<vxe-table-column type="checkbox" width="28" :resizable="false"></vxe-table-column>
			<vxe-table-column sortable field="key" width="21%" title="键" show-header-overflow show-overflow></vxe-table-column>
			<vxe-table-column sortable field="value" width="10%" title="值" :edit-render="{name: 'input'}" show-header-overflow show-overflow></vxe-table-column>
			<vxe-table-column sortable field="defaultValue" width="10%" title="默认值" show-header-overflow show-overflow></vxe-table-column>
			<vxe-table-column sortable field="range" width="11%"  title="范围" show-header-overflow show-overflow></vxe-table-column>
			<vxe-table-column sortable field="canSet" width="7%" title="允许编辑" show-header-overflow show-overflow>
				<template v-slot="{ row }">
					<el-switch v-model="row.canSet" disabled></el-switch>
					<span class="enabledText disabledText">{{ row.canSet ? "是":"否" }}</span>
				</template>
			</vxe-table-column>
			<vxe-table-column sortable field="mustRestart" width="7%"  title="重启生效" show-header-overflow show-overflow>
				<template v-slot="{ row }">
					<el-switch v-model="row.mustRestart" disabled></el-switch>
					<span class="enabledText disabledText">{{ row.mustRestart ? "是":"否" }}</span>
				</template>
			</vxe-table-column>
			<vxe-table-column sortable field="desc" title="描述" show-header-overflow show-overflow></vxe-table-column>
		</vxe-table>
...
<script type="text/javascript" src="js/servgroup/mysqls/manageTab/paramCfg/list.js?t=<%=new Date().getTime() %>"></script>
