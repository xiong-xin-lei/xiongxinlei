...
<body>
    <div id="list" v-cloak>
        ...
        <div class="iframeBorder">
            ...
            <vxe-table size="small" highlight-hover-row resizable auto-resize border="inner" :max-height="table_heighth"
                style="color: #333333;" :tooltip-config="{theme: 'light'}" :align="allAlign" row-id="id" ref="xTable"
                :data="tableData" :sort-config="{trigger: 'cell',remote : 'true'}"
                :checkbox-config="{highlight: true, trigger: 'row'}" :resizable-config="{minWidth: resizableMinWidth}"
                :row-class-name="tableRowClassName" @sort-change="sortChange" @checkbox-change="selectChangeEvent"
                @cell-mouseenter="cellMouseenterEvent" @cell-mouseleave="cellMouseleaveEvent"
                @checkbox-all="selectAllEvent" @cell-dblclick="cellDblClickManage">
                <vxe-table-column type="checkbox" width="28" :resizable="false"></vxe-table-column>
                <vxe-table-column sortable field="name" title="服务名称" show-header-overflow show-overflow width="7%">
                	<template v-slot="{ row }">
                ...
                <vxe-table-column field="version" v-bind="commonMinWidth('版本架构')" show-header-overflow width="6%">
                    <template v-slot="{ row }">
                        <div>
                            ...
                            {{ jsonJudgeNotDefined(row,'row.sysArchitecture.display') }}<br>
                        </div>
                    </template>
                </vxe-table-column>
                <vxe-table-column field="businessArea" v-bind="commonMinWidth('规模架构')" show-header-overflow width="11%">
                    <template v-slot="{ row }">
                        <div>
                ...
                <vxe-table-column field="addresses" v-bind="commonMinWidth('服务地址')" show-header-overflow width="10%">
                    <template v-slot="{ row }">
                    	<template v-if="row.addresses.length < 4">
                        ...
                        <el-tooltip v-else placement="right">
							...
							<div>
								<span>{{row.addresses[0]}}</span><br>
	                        	<span>{{row.addresses[1]}}</span><br>
	                        	<span>......</span>
							</div>
						</el-tooltip>
                    </template>
...
<script type="text/javascript" src="js/servgroup/redis/list.js?t=<%=new Date().getTime() %>"></script>
