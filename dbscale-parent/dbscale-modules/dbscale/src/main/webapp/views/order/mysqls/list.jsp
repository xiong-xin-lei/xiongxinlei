    ...
    <div id="mysqlList" v-cloak>
        ...
        <div class="iframeBorder">
            ...
            <vxe-table size="small" :max-height="table_heighth" highlight-hover-row resizable auto-resize border="inner"
                ...
                <vxe-table-column sortable field="name" v-bind="commonMinWidth('工单名')" show-header-overflow
                    show-overflow width="6%">
                    <template v-slot="{ row }">
                ...
                <vxe-table-column sortable field="ownerName" v-bind="commonMinWidth('所属者')" show-header-overflow
                    show-overflow width="9%">
                </vxe-table-column>
                <vxe-table-column field="businessSubsystem" v-bind="commonMinWidth('业务系统')" show-header-overflow
                ...
                <vxe-table-column v-bind="commonMinWidth('配置信息')" show-header-overflow>
                    <template v-slot="{ row }">
                        <div v-if="row.createType.code === 'new'">
                            ...
                            <span style="display: inline-block;margin-right: 3%;vertical-align: middle;width: 35%;">
                                规模：{{ jsonJudgeNotDefined(row, "row.orders.scale.name") }}<br>
                                架构：{{ jsonJudgeNotDefined(row, "row.orders.arch.mode.display") }}
                                <span v-if="row.orders.arch.mode.display!=='单节点'">({{ row.unitCntText }})</span>
                ...
                <vxe-table-column sortable field="createType.display" v-bind="commonMinWidth('工单类型')"
                    show-header-overflow show-overflow width="7%"></vxe-table-column>
                <vxe-table-column sortable field="status.display" v-bind="commonMinWidth('状态')" type="html"
                    ...
                    <template v-slot="{ row }">
                        <template v-if="jsonJudgeNotDefined(row,'row.state.code')!=='unapprove'">
                        ...
                        <template v-else>
                            <el-tooltip placement="right">
                                <div slot="content">
                                    <span>审批信息：</span><span style="padding-left:5px">{{ row.msg }}</span>
                                </div>
                                <span v-if="row.taskId===null" v-html="statusShow(row.status)"></span>
                                <a v-else @click="actionDisplayClick(row)" class="taskStatusStyle" v-html="statusShow(row.status)"></a>
                            </el-tooltip>
...
<script type="text/javascript" src="js/order/mysqls/list.js?t=<%=new Date().getTime() %>"></script>
