...
<div id="list" v-cloak>
    ...
    <div class="iframeBorder">
        ...
        <vxe-table size="small" :max-height="table_heighth" highlight-hover-row resizable auto-resize border="inner" style="color: #333333;"
            ...
            <vxe-table-column sortable field="type.display" v-bind="commonMinWidth('软件类型')" show-header-overflow show-overflow></vxe-table-column>
            <vxe-table-column sortable field="version" v-bind="commonMinWidth('软件版本')" show-header-overflow show-overflow></vxe-table-column>
            <vxe-table-column sortable field="architecture.display" v-bind="commonMinWidth('架构')" show-header-overflow show-overflow></vxe-table-column>
            <vxe-table-column sortable field="statefulText" v-bind="commonMinWidth('服务状态')" show-header-overflow show-overflow>
     			<template v-slot="{ row }">
                    <el-tag v-if="row.stateful === true" effect="dark" color="#00ABAC" style="text-align: center;height: 24px;line-height: 24px;border: none;">{{ row.statefulText }}</el-tag>
                    <el-tag v-else-if="row.stateful === false" effect="dark" color="#326CE5" style="text-align: center;height: 24px;line-height: 24px;border: none;">{{ row.statefulText }}</el-tag>
                    <span v-else>{{ row.statefulText }}</span>
				</template>
            </vxe-table-column>
            <vxe-table-column sortable field="gmtCreated" v-bind="commonMinWidth('创建时间')" show-header-overflow show-overflow></vxe-table-column>
...
<script type="text/javascript" src="js/resource/images/list.js?t=<%=new Date().getTime() %>"></script>
