...
<div id="list" v-cloak>
    ...
    <el-breadcrumb separator="/" style="padding: 20px 0;">
        <el-breadcrumb-item>
        	<span style="color:#999;font-size: 12px;"><i class="el-icon-message"></i>{{ menu.name }}</span>
        </el-breadcrumb-item>
        <el-breadcrumb-item>
        	<span style="color:#999;font-size: 12px;">{{ menu.subMenu.name }} ({{remoteStorageName}})</span>
        </el-breadcrumb-item>
        <el-breadcrumb-item>
    ...
    <div class="iframeBorder">
        ...
        <vxe-table size="small" :max-height="table_heighth" highlight-hover-row resizable auto-resize border="inner" style="color: #333333;"
            ...
            @sort-change="sortChange"  :sort-config="{trigger: 'cell',remote : 'true'}" :resizable-config="{minWidth: resizableMinWidth}"
            :row-class-name="tableRowClassName">
            <vxe-table-column type="checkbox" width="28" :resizable="false"></vxe-table-column>
            <vxe-table-column sortable field="name" v-bind="commonMinWidth('存储名称')" show-header-overflow show-overflow></vxe-table-column>
            <vxe-table-column sortable field="remoteStorage.name" v-bind="commonMinWidth('存储池名称')" show-header-overflow show-overflow></vxe-table-column>
            <vxe-table-column sortable field="performance.display" v-bind="commonMinWidth('性能等级')" show-header-overflow show-overflow></vxe-table-column>
            <vxe-table-column sortable field="usedCapacity" v-bind="commonMinWidth('分配率')" show-header-overflow show-overflow></vxe-table-column>
            <vxe-table-column sortable field="enabled" v-bind="commonMinWidth('是否可用')" type="html" show-header-overflow show-overflow width="90px">
				<template v-slot="{ row }">
					<div v-html="statusShow(row.enabled,'enabled')"></div>
				</template>
            </vxe-table-column>
...
<script type="text/javascript">
	...
	<%Object remoteStorageId = request.getAttribute("remoteStorageId");%>
	var remoteStorageId = <%=remoteStorageId%>
</script>

<script type="text/javascript" src="js/resource/storage/volumepaths/RGmanage/list.js?t=<%=new Date().getTime() %>"></script>
