...
<div id="list" v-cloak>
    ...
    <div class="iframeBorder">
        ...
        <vxe-table size="small" :max-height="table_heighth" highlight-hover-row resizable auto-resize border="inner" style="color: #333333;"
            ...
            <vxe-table-column type="expand" width="0" :resizable="false">
				<template v-slot:content="{ row }">
	            	<el-row style="margin: 10px 40px">
	            		<el-col :span="6">
	            			<span>高可用标签：</span><span style="padding-left:5px">{{ row.childRows.haTag }}</span>
	            		</el-col>
	            		<el-col :span="6">
            ...
            <vxe-table-column sortable field="name" v-bind="commonMinWidth('集群名称')" width="15%" show-header-overflow show-overflow>
            	<template v-slot="{ row }">
            ...
            <vxe-table-column field="imageTypes" v-bind="commonMinWidth('包含软件')" show-header-overflow >
            	<template v-slot="{ row }">
					<template v-for="listdata in row.imageTypes">
                        <el-tag class="imageTypesTag" v-if="listdata.stateful === true" effect="dark" color="#00ABAC" style="text-align: center;height: 24px;line-height: 24px;border: none;">{{ listdata.name }}</el-tag>
                        <el-tag class="imageTypesTag" v-else-if="listdata.stateful === false" effect="dark" color="#326CE5" style="text-align: center;height: 24px;line-height: 24px;border: none;">{{ listdata.name }}</el-tag>
                        <span class="imageTypesTag" v-else>{{ listdata.name }}</span>
                    </template>
				</template>
            </vxe-table-column>
          <!--   <vxe-table-column field="networks" type="html" v-bind="commonMinWidth('关联网段')" show-header-overflow width="15%">
            	<template v-slot="{ row }">
					<div v-for="listdata in row.networks">
						<a v-if="listdata.enabled" class="taskStatusStyle" @click="aAssociatedJumpClick(listdata.name, '关联网段')" >
	                        <span style="color:#19be6b;border-bottom: 1px solid #19be6b;">
	                            {{ listdata.name }}
	                        </span>
                        </a>
                        <el-tooltip v-else  placement="right">
							<div slot="content">
								<span><span class="tooltipText">是否可用：</span>{{ listdata.enabled ? "是" : "否"}}</span>
							</div>
							<a class="taskStatusStyle" @click="aAssociatedJumpClick(listdata.name,'关联网段')" >
		                    	<span style="color:red;border-bottom: 1px solid red;">
								    {{ listdata.name }}
								</span>
							</a>
...
<script type="text/javascript" src="js/resource/clusters/list.js?t=<%=new Date().getTime() %>"></script>
