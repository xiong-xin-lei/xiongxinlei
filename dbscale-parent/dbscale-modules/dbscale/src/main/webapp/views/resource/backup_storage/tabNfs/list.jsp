...
<body>
    ...
    <div class="iframeBorder">
        ...
        <vxe-table size="small" :max-height="table_heighth" highlight-hover-row resizable auto-resize border="inner" style="color: #333333;"
            ...
            <vxe-table-column sortable field="name" v-bind="commonMinWidth('名称')" show-header-overflow show-overflow></vxe-table-column>
            <vxe-table-column sortable field="businessArea.name" v-bind="commonMinWidth('所属业务区')" show-header-overflow show-overflow>
            	<template v-slot="{ row }">
            ...
            <vxe-table-column sortable field="nfsIp" v-bind="commonMinWidth('NFS地址')" show-header-overflow show-overflow></vxe-table-column>
            <vxe-table-column sortable field="nfsSource" v-bind="commonMinWidth('源目录')" show-header-overflow show-overflow></vxe-table-column>
            <vxe-table-column sortable field="nfsOpts" v-bind="commonMinWidth('挂载参数')" show-header-overflow show-overflow></vxe-table-column>
            <vxe-table-column sortable field="description" v-bind="commonMinWidth('描述')" show-header-overflow show-overflow></vxe-table-column>
            <vxe-table-column sortable field="state.display" v-bind="commonMinWidth('状态')" type="html" show-header-overflow show-overflow width="7%">
                <template v-slot="{ row }">
                    <span v-html="statusShow(row,'state')"></span><br/>
                </template>
            </vxe-table-column>
            <vxe-table-column sortable field="enabledText" v-bind="commonMinWidth('是否可用')" type="html" show-header-overflow show-overflow width="90px">
				<template v-slot="{ row }">
...
<script type="text/javascript" src="js/resource/backup_storage/tabNfs/list.js?t=<%=new Date().getTime() %>"></script>
