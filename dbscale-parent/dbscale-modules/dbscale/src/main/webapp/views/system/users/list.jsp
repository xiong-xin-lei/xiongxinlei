...
<div id="list" v-cloak>
    ...
    <div class="iframeBorder">
        ...
        <vxe-table size="small" highlight-hover-row resizable auto-resize border="inner" :max-height="table_heighth" style="color: #333333;"
            ...
            <vxe-table-column sortable field="username" v-bind="commonMinWidth('登录用户名')" show-header-overflow show-overflow></vxe-table-column>
            <vxe-table-column sortable field="name" v-bind="commonMinWidth('用户姓名')" show-header-overflow show-overflow></vxe-table-column>
            <vxe-table-column sortable field="company" v-bind="commonMinWidth('所属单位')" show-header-overflow show-overflow></vxe-table-column>
            <vxe-table-column sortable field="telephone" v-bind="commonMinWidth('联系电话')" show-header-overflow show-overflow></vxe-table-column>
            <vxe-table-column sortable field="email" v-bind="commonMinWidth('邮箱')" show-header-overflow show-overflow width="14%"></vxe-table-column>
            <vxe-table-column sortable field="role.name" v-bind="commonMinWidth('角色')" show-header-overflow show-overflow></vxe-table-column>
            <vxe-table-column field="groupsName" v-bind="commonMinWidth('组别')" show-header-overflow>
                <template v-slot="{ row }">
                    <div v-for="list in row.groupsName"><span>{{ list }}</span></div>
                </template>
            </vxe-table-column>
            <vxe-table-column sortable field="created.timestamp" v-bind="commonMinWidth('注册时间')" show-header-overflow show-overflow width="10%"></vxe-table-column>
            <vxe-table-column sortable field="ogAutoExamineText" v-bind="commonMinWidth('自动审批')" type="html" show-header-overflow show-overflow width="90px">
                <template v-slot="{ row }">
                    <div v-html="statusShow(row.ogAutoExamine)"></div>
                </template>
            </vxe-table-column>
            <vxe-table-column sortable field="ogAutoExecuteText" v-bind="commonMinWidth('自动执行')" type="html" show-header-overflow show-overflow width="90px">
                <template v-slot="{ row }">
                    <div v-html="statusShow(row.ogAutoExecute)"></div>
                </template>
            </vxe-table-column>
            <vxe-table-column sortable field="statusText" v-bind="commonMinWidth('是否可用')" type="html" show-header-overflow show-overflow width="90px">
				<template v-slot="{ row }">
...
<script type="text/javascript" src="js/system/users/list.js?t=<%=new Date().getTime() %>"></script>
