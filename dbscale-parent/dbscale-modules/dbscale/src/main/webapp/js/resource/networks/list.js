    ...
    methods: {
        dataDispose: function (data) {
            ...
            XEUtils.arrayEach(data.data, (v, i) => {
                ...
                json.IpUsed = usedCapacityDispose(v.ipUsed, v.ipTotal)
        ...
        addAjaxFun: function (getCheckboxTablerId) {
            ...
            var height = '490px'
            var title = "新增"
            var url = "/" + getProjectName() + "/app/resource/networks/add"
            var urlData = ""
        ...
        updataAjaxFun: function (getCheckboxTablerId) {
            ...
            var height = '490px'
            var title = "编辑"
            var url = "/" + getProjectName() + "/app/resource/networks/edit/" + getCheckboxTablerId.id
            var urlData = ""
            layerOpenFun(width, height, title, url + urlData)
        },
        startAjaxFun: function (getCheckboxTablerId, successFun, falseFun) {
            var _this = this
            sendPut("/" + getProjectName() + "/networks/" + getCheckboxTablerId.id + "/enabled", function (response) {
        ...
        stopAjaxFun: function (getCheckboxTablerId, successFun, falseFun) {
            var _this = this
            sendPut("/" + getProjectName() + "/networks/" + getCheckboxTablerId.id + "/disabled", function (response) {
                successFun(response, getCheckboxTablerId)
                _this.refreshSaveData.push(getCheckboxTablerId)
            }, function (error) {
                falseFun(error, getCheckboxTablerId)
        ...
        deleteAjaxFun: function (getCheckboxTablerId, successFun, falseFun) {
            var _this = this
            sendDelete("/" + getProjectName() + "/networks/" + getCheckboxTablerId.id, function (response) {
                successFun(response, getCheckboxTablerId)
        ...
        refreshAjaxFun: function (getCheckboxTablerId, successFun, falseFun) {
            var _this = this
            sendGet("/" + getProjectName() + "/networks?site_id=" + getSession("siteId"), function (response) {
                successFun(response, getCheckboxTablerId)
        ...
        btnClick: function (code, array, getCheckboxTablerId) {
            ...
            this.updateStatus = ''
            let confirmsName = array ? ['网段'] : getCheckboxTablerId.name
        ...
        loadContentMethod({row}) {
            ...
            return new Promise(resolve => {
                sendGet("/" + getProjectName() + "/networks/" + row.id, function (response) {
                    layer.closeAll('loading')
        ...
        childDataDispose: function (v) {
            ...
            json.topologys = arrayMerger(v.topologys, "display")
            json.createdName = ownerNameDispose(v.created.name, v.created.username)

            return json
        },
        searchClick: function (settings) {
            let keyArray = ['name', 'startIp', 'endIp', 'gateway', 'netmask', 'IpUsed', 'cluster.name', 'statusText']
