var listApp = new Vue({
    ...
    methods: {
        ...
        addAjaxFun: function (getCheckboxTablerId) {
            ...
            var height = '335px'
            var title = "新增"
            var url = "/" + getProjectName() + "/app/resource/storage/pvcs/add"
            var urlData = ""
        ...
        updataAjaxFun: function (getCheckboxTablerId) {
            ...
            var height = '335px'
            var title = "编辑"
            var url = "/" + getProjectName() + "/app/resource/storage/pvcs/edit/" + getCheckboxTablerId.id
            var urlData = ""
            layerOpenFun(width, height, title, url + urlData)
        },
        startAjaxFun: function (getCheckboxTablerId, successFun, falseFun) {
            var _this = this
            sendPut("/" + getProjectName() + "/pvc_storages/" + getCheckboxTablerId.id + "/enabled", function (response) {
        ...
        stopAjaxFun: function (getCheckboxTablerId, successFun, falseFun) {
            var _this = this
            sendPut("/" + getProjectName() + "/pvc_storages/" + getCheckboxTablerId.id + "/disabled", function (response) {
                successFun(response, getCheckboxTablerId)
                _this.refreshSaveData.push(getCheckboxTablerId)
            }, function (error) {
                falseFun(error, getCheckboxTablerId)
        ...
        deleteAjaxFun: function (getCheckboxTablerId, successFun, falseFun) {
            var _this = this
            sendDelete("/" + getProjectName() + "/pvc_storages/" + getCheckboxTablerId.id, function (response) {
                successFun(response, getCheckboxTablerId)
        ...
        refreshAjaxFun: function (getCheckboxTablerId, successFun, falseFun) {
            var _this = this
            sendGet("/" + getProjectName() + "/pvc_storages?site_id=" + getSession("siteId"), function (response) {
                successFun(response, getCheckboxTablerId)
        ...
        searchClick: function (settings) {
            let keyArray = ['name', 'provisioner', 'reclaimPolicy.display', 'volumeBingdingType.display', 'allowVolumeExpansion', 'created.timestamp', 'description', 'statusText']
