...
var checkNum = (rule, value, callback) => {
    if (editScaleUp.formData.orderType === 'mysql') {
        if (value < 10) {
        ...
        if (value < 5) {
            callback(new Error('最小值为5 '));
            return false;
        }
    }
    callback();
};
var editScaleUp = new Vue({
    el: '#editScaleUp',
    data: {
        ...
        formRules: {
            ...
            dataSize: [
                {required: true, message: '请输入空间大小', trigger: ['change', 'blur']},
                {validator: checkNum, trigger: 'blur'}
            ],
            logSize: [
                {required: true, message: '请输入大小', trigger: ['change', 'blur']},
                {validator: checkNum, trigger: 'blur'}
            ]
        },
        dataShow: function () {
        ...
        formSubmit: function (formName) {//editScaleUp
            ...
            const jsonData = {
                "orders": [
                    {
                        "type": orderType,
                        "dataSize": parseInt(dataDirSize),
                        "logSize": parseInt(logDirSize)
                    }
                ]
            }
