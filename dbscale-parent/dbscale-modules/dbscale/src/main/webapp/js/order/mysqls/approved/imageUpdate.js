var approvedImageUpdateIndex = parent.layer.getFrameIndex(window.name);
var ogAutoExecute = getQueryVariable("ogAutoExecute") === "true"
new Vue({
    el: '#approvedImageUpdate',
    data: {
    ...
    methods: {
            ...
            sendGet("/" + getProjectName() + "/order_groups/" + approvedId, function (response) {
                ...
                XEUtils.arrayEach(data.orders, (v, i) => {
                    if (v.version != null) {
                        ...
                        var version = v.version.major + '.' + v.version.minor + '.' + v.version.patch + '.' + v.version.build
        ...
        formSubmit: function (formName, type) {//approvedImageUpdate
        ...
        formClose: function () {
            parent.layer.close(approvedImageUpdateIndex);
        }
    }
})