var listApp = new Vue({
    ...
    created: function () {
        ...
        setTimeout(function () {
            getHtmlMinWidth("mysqlList", _this, ['xTable'])
        }, 0)
    },
    methods: {
            ...
            XEUtils.arrayEach(data.data, (v, i) => {
                ...
                if (v.orders.length === 0 || v.orders === null) {
                    ...
                    XEUtils.arrayEach(v.orders, (value, index) => {
                        ...
                        } else if (v.createType.code === "delete") {
                            cfgTitle = ""
                        }
        ...
        refreshAjaxFun: function (getCheckboxTablerId, successFun, falseFun) {
            var _this = this
            sendGet("/" + getProjectName() + "/order_groups?site_id=" + getSession("siteId") + "&category=mysql", function (response) {
                successFun(response, getCheckboxTablerId);
                layer.closeAll('loading');
            }, function (error) {
                falseFun(error, getCheckboxTablerId)
        ...
        btnClick: function (code, array, getCheckboxTablerId) {
            ...
            switch (code) {
                case this.btnReplaceList.add://新增
                    sendGet("/" + getProjectName() + "/orders/cfgs/mysql", function (response) {
                        var data = response.data.data
                        layer.closeAll('loading')
                        if (data.length !== 0) {
                            _this.userListView(function () {
                                width = 778
                                height = 485
                                url = "/" + getProjectName() + "/app/order/mysqls/add"
                ...
                case this.btnReplaceList.jump://跳转
                    ...
                    var urlCode = "/app/servgroup/mysqls"
                    ...
                    url = "/app/servgroup/mysqls/manage/" + indexId + "?id=" + id + "&menu=" + encodeURIComponent(JSON.stringify(menuData), "utf-8") + "&highAvailable=" + getCheckboxTablerId.highAvailable
                    json = {
                        code: url,
                        name: indexName,
                    }
                ...
                case this.btnReplaceList.update://编辑
                    _this.userListView(function () {
                        ...
                        switch (getCheckboxTablerId.createType.code) {
                            case "new"://新工单
                                url = "/" + getProjectName() + "/app/order/mysqls/edit/new/" + getCheckboxTablerId.id
                                _this.editAjaxFun(getCheckboxTablerId, width + "px", height + "px", url + urlData)
                                break;
                            case "scale_up_cpumem"://计量扩容
                                url = "/" + getProjectName() + "/app/order/mysqls/edit/scaleUp/cpumem/" + getCheckboxTablerId.id
                                _this.editAjaxFun(getCheckboxTablerId, width + "px", height + "px", url + urlData)
                                break;
                            case "scale_up_storage"://存储扩容
                                url = "/" + getProjectName() + "/app/order/mysqls/edit/scaleUp/storage/" + getCheckboxTablerId.id
                                _this.editAjaxFun(getCheckboxTablerId, width + "px", height + "px", url + urlData)
                                break;
                            case "image_update"://升级
                                url = "/" + getProjectName() + "/app/order/mysqls/edit/imageUpdate/" + getCheckboxTablerId.id
                                _this.editAjaxFun(getCheckboxTablerId, width + "px", height + "px", url + urlData)
                                break;
                        }
                    })
                ...
                case this.btnReplaceList.approved://审批
                    _this.userListView(function () {
                        ...
                        switch (getCheckboxTablerId.createType.code) {
                            case "new"://新工单
                                url = "/" + getProjectName() + "/app/order/mysqls/approved/new/" + getCheckboxTablerId.id
                                _this.approvedAjaxFun(getCheckboxTablerId, width + "px", height + "px", url + urlData)
                                break;
                            case "scale_up_cpumem"://计量扩容
                                url = "/" + getProjectName() + "/app/order/mysqls/approved/scaleUp/cpumem/" + getCheckboxTablerId.id
                                _this.approvedAjaxFun(getCheckboxTablerId, width + "px", height + "px", url + urlData)
                                break;
                            case "scale_up_storage"://存储扩容
                                url = "/" + getProjectName() + "/app/order/mysqls/approved/scaleUp/storage/" + getCheckboxTablerId.id
                                _this.approvedAjaxFun(getCheckboxTablerId, width + "px", height + "px", url + urlData)
                                break;
                            case "image_update"://升级
                                url = "/" + getProjectName() + "/app/order/mysqls/approved/imageUpdate/" + getCheckboxTablerId.id
                                _this.approvedAjaxFun(getCheckboxTablerId, width + "px", height + "px", url + urlData)
                                break;
                            case "delete"://删除
                                url = "/" + getProjectName() + "/app/order/mysqls/approved/delete/" + getCheckboxTablerId.id
                                _this.approvedAjaxFun(getCheckboxTablerId, width + "px", height + "px", url + urlData)
                                break;
                        }
                    })
                ...
                case this.btnReplaceList.execute://执行
                    if (array) {
                        ...
                        switch (getCheckboxTablerId.createType.code) {
                            case "new"://新工单
                                url = "/" + getProjectName() + "/app/order/mysqls/execute/new/" + getCheckboxTablerId.id
                                _this.executeAjaxFun(getCheckboxTablerId, '778px', '485px', url)
                                break;
                            case "scale_up_cpumem"://计量扩容
                                url = "/" + getProjectName() + "/app/order/mysqls/execute/scaleUp/cpumem/" + getCheckboxTablerId.id
                                _this.executeAjaxFun(getCheckboxTablerId, '778px', '485px', url)
                                break;
                            case "scale_up_storage"://存储扩容
                                url = "/" + getProjectName() + "/app/order/mysqls/execute/scaleUp/storage/" + getCheckboxTablerId.id
                                _this.executeAjaxFun(getCheckboxTablerId, '778px', '485px', url)
                                break;
                            case "image_update"://升级
                                url = "/" + getProjectName() + "/app/order/mysqls/execute/imageUpdate/" + getCheckboxTablerId.id
                                _this.executeAjaxFun(getCheckboxTablerId, '778px', '485px', url)
                                break;
                            case "delete"://删除
                                url = "/" + getProjectName() + "/app/order/mysqls/execute/delete/" + getCheckboxTablerId.id
                                _this.executeAjaxFun(getCheckboxTablerId, '778px', '485px', url)
                                break;
                        }
                    }
        ...
        searchClick: function (settings) {
            let keyArray = ['name', 'ownerName', 'businessSubsystem.name', 'businessSubsystem.businessSystem.name', 'site.name',
                ...
                'unitCntText', 'highAvailableText', 'orders.diskType.display', 'dataSize', 'logSize', 'createType.display', 'state.display',]
