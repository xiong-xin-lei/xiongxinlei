...
new Vue({
    ...
    data: {
        ...
        formData: {
            ...
            proxysqldataSize: '',
            proxysqllogSize: '',
            /*  proxysqlclusterHA: false,
              proxysqlhostHA: false,
 ...
 */
            ...
            if (_this.judgeNull) {
                commonConfirm(_this, '新增确认', getHintText('新增')).then(() => {
                    sendPost("/" + getProjectName() + "/orders/cfgs/mysql", function (response) {
                        layer.closeAll('loading')
                        _this.judgeNull = false
                        operationCompletion(_this, "工单配置  MySQL 新增成功！", "success")
                    }, function (error) {
                        operationCompletion(_this, error.response.data.msg, "error")
                    }, jsonData)
                })
                ...
                commonConfirm(_this, '编辑确认', getHintText('编辑')).then(() => {
                    sendPut("/" + getProjectName() + "/orders/cfgs/mysql", function (response) {
                        layer.closeAll('loading')
                        operationCompletion(_this, "工单配置  MySQL 编辑成功！", "success")
                    }, function (error) {
                        operationCompletion(_this, error.response.data.msg, "error")
                    }, jsonData)
                })
