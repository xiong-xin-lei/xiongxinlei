...
var validateFileRetentionNum = (rule, value, callback) => {
    var timeType = ""
    switch (mysqlEditApp.formData.cronExpression) {
        case 'day':
            timeType = mysqlEditApp.formData.time !== null
            break;
        case 'week':
            timeType = mysqlEditApp.formData.time !== null && mysqlEditApp.formData.week !== null
            break;
        case 'month':
            timeType = mysqlEditApp.formData.time !== null && mysqlEditApp.formData.month !== null
            break;
    }
    var All = mysqlEditApp.formData.backupStorageType === '' && mysqlEditApp.formData.backUpType === '' &&
        mysqlEditApp.formData.cronExpression === '' && mysqlEditApp.formData.month === null &&
        mysqlEditApp.formData.week === null && mysqlEditApp.formData.time === null &&
        mysqlEditApp.formData.fileRetentionNum === undefined

    var FullAll = mysqlEditApp.formData.backupStorageType !== '' && mysqlEditApp.formData.backUpType !== '' &&
        mysqlEditApp.formData.cronExpression !== '' && mysqlEditApp.formData.fileRetentionNum !== undefined && timeType
...
var mysqlEditApp = new Vue({
    ...
    data: {
        ...
        formRules: {
            ...
            proxyNum: [
                {required: true, message: '请输入代理数量', trigger: ['change', 'blur']}
            ],
            backupStorageType: [
    ...
    methods: {
        ...
        formSubmit: function (formName, type) {//editNew
            ...
            if (highAvailable) {
                var proxyJson = {
                    "type": "proxysql",
                    "version": _this.proxyVersionData
                }
                var proxyArchMode = ""
