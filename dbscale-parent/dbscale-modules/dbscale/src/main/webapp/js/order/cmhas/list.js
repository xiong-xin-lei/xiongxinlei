var listApp = new Vue({
    ...
    created: function () {
        ...
        setTimeout(function () {
            getHtmlMinWidth("cmhaList", _this, ['xTable'])
        }, 0)
    },
    methods: {
        ...
        refreshAjaxFun: function (getCheckboxTablerId, successFun, falseFun) {
            var _this = this
            sendGet("/" + getProjectName() + "/order_groups?site_id=" + getSession("siteId") + "&category=cmha", function (response) {
                successFun(response, getCheckboxTablerId);
                layer.closeAll('loading');
            }, function (error) {
                falseFun(error, getCheckboxTablerId)
        ...
        btnClick: function (code, array, getCheckboxTablerId) {
            ...
            switch (code) {
                    ...
                    sendGet("/" + getProjectName() + "/orders/cfgs/cmha", function (response) {
                        ...
                        if (data.length !== 0) {
                            _this.userListView(function () {
                                width = 785
                                height = 490
                                url = "/" + getProjectName() + "/app/order/cmhas/add"
                ...
                case this.btnReplaceList.jump://跳转
                    ...
                    var urlCode = "/app/servgroup/cmhas"
                    ...
                    url = "/app/servgroup/cmhas/manage/" + indexId + "?id=" + id + "&menu=" + encodeURIComponent(JSON.stringify(menuData), "utf-8")
                    json = {
                        code: url,
                        name: indexName,
                    }
                ...
                case this.btnReplaceList.update:// 编辑
                    _this.userListView(function () {
                        width = 778
                        height = 485
                        if (getCheckboxTablerId.createType.code === 'new') {
                            height = 535
                        }
                        ...
                        switch (getCheckboxTablerId.createType.code) {
                            case "new":// 新工单
                                url = "/" + getProjectName() + "/app/order/cmhas/edit/new/" + getCheckboxTablerId.id
                                _this.editAjaxFun(getCheckboxTablerId, width + "px", height + "px", url + urlData)
                                break;
                            case "scale_up_cpumem":// 计量扩容
                                url = "/" + getProjectName() + "/app/order/cmhas/edit/scaleUp/cpumem/" + getCheckboxTablerId.id
                                _this.editAjaxFun(getCheckboxTablerId, width + "px", height + "px", url + urlData)
                                break;
                            case "scale_up_storage":// 存储扩容
                                url = "/" + getProjectName() + "/app/order/cmhas/edit/scaleUp/storage/" + getCheckboxTablerId.id
                                _this.editAjaxFun(getCheckboxTablerId, width + "px", height + "px", url + urlData)
                                break;
                            case "image_update":// 升级
                                url = "/" + getProjectName() + "/app/order/cmhas/edit/imageUpdate/" + getCheckboxTablerId.id
                                _this.editAjaxFun(getCheckboxTablerId, width + "px", height + "px", url + urlData)
                                break;
                        }
                    })
                ...
                case this.btnReplaceList.approved:// 审批
                    _this.userListView(function () {
                        ...
                        switch (getCheckboxTablerId.createType.code) {
                            case "new":// 新工单
                                url = "/" + getProjectName() + "/app/order/cmhas/approved/new/" + getCheckboxTablerId.id
                                _this.approvedAjaxFun(getCheckboxTablerId, width + "px", height + "px", url + urlData)
                                break;
                            case "scale_up_cpumem":// 计量扩容
                                url = "/" + getProjectName() + "/app/order/cmhas/approved/scaleUp/cpumem/" + getCheckboxTablerId.id
                                _this.approvedAjaxFun(getCheckboxTablerId, width + "px", height + "px", url + urlData)
                                break;
                            case "scale_up_storage":// 存储扩容
                                url = "/" + getProjectName() + "/app/order/cmhas/approved/scaleUp/storage/" + getCheckboxTablerId.id
                                _this.approvedAjaxFun(getCheckboxTablerId, width + "px", height + "px", url + urlData)
                                break;
                            case "image_update":// 升级
                                url = "/" + getProjectName() + "/app/order/cmhas/approved/imageUpdate/" + getCheckboxTablerId.id
                                _this.approvedAjaxFun(getCheckboxTablerId, width + "px", height + "px", url + urlData)
                                break;
                            case "delete":// 删除
                                url = "/" + getProjectName() + "/app/order/cmhas/approved/delete/" + getCheckboxTablerId.id
                                _this.approvedAjaxFun(getCheckboxTablerId, width + "px", height + "px", url + urlData)
                                break;
                        }
                    })
                ...
                case this.btnReplaceList.execute:// 执行
                    if (array) {
                        ...
                        switch (getCheckboxTablerId.createType.code) {
                            case "new":// 新工单
                                url = "/" + getProjectName() + "/app/order/cmhas/execute/new/" + getCheckboxTablerId.id
                                _this.executeAjaxFun(getCheckboxTablerId, '778px', '535px', url)
                                break;
                            case "scale_up_cpumem":// 计量扩容
                                url = "/" + getProjectName() + "/app/order/cmhas/execute/scaleUp/cpumem/" + getCheckboxTablerId.id
                                _this.executeAjaxFun(getCheckboxTablerId, '778px', '485px', url)
                                break;
                            case "scale_up_storage":// 存储扩容
                                url = "/" + getProjectName() + "/app/order/cmhas/execute/scaleUp/storage/" + getCheckboxTablerId.id
                                _this.executeAjaxFun(getCheckboxTablerId, '778px', '485px', url)
                                break;
                            case "image_update":// 升级
                                url = "/" + getProjectName() + "/app/order/cmhas/execute/imageUpdate/" + getCheckboxTablerId.id
                                _this.executeAjaxFun(getCheckboxTablerId, '778px', '485px', url)
                                break;
                            case "delete":// 删除
                                url = "/" + getProjectName() + "/app/order/cmhas/execute/delete/" + getCheckboxTablerId.id
                                _this.executeAjaxFun(getCheckboxTablerId, '778px', '485px', url)
                                break;
                        }
                    }
        ...
        childDataDispose: function (v) {
            ...
            var fileRetentionNum = ""
            var description = ""
            var paramCfg = ""
            var fileRetentionNumText = ""
            var cronExpressionArray = ""
            var cronExpressionTimeText = ""
            var jsonArray = []
            var proxyNum = ""
                    ...
                    if (value.type.code === "mysql") {
                        if (v.createType.code === "new" && value.cfg !== null) {
                            ...
                            if (value.cfg.backupStrategy !== undefined && value.cfg.backupStrategy !== null) {
                                ...
                                fileRetentionNumText = jsonJudgeNotDefined(value, "value.cfg.backupStrategy.fileRetentionNum") + ""
                                cronExpressionArray = value.cfg.backupStrategy.cronExpression.split(' ')
                                cronExpressionText = ""
                                cronExpressionTimeText = cronExpressionArray[1] + ":" + cronExpressionArray[0]
                        ...
                        paramCfg = jsonArray
                    } else if (value.type.code === "proxysql") {
                        proxyNum = jsonJudgeNotDefined(value, "value.arch.unitCnt")
                    }
                })
            }
        ...
        searchClick: function () {
            let keyArray = ['name', 'ownerName', 'businessSubsystem.name', 'businessSubsystem.businessSystem.name', 'site.name',
                'businessArea.name', 'createType.display', 'state.display', 'orderVersion', 'orders.arch.name',
                'orders.scale.name', 'sysArchitecture.display', 'orders.diskType.display', 'dataSize', 'logSize', 'cfgTitle']
