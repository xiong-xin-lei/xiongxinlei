...
var mysqlListApp = new Vue({
    ...
    methods: {
                ...
                if (value.type.code === 'mysql') {
                    XEUtils.arrayEach(value.units, (v, i) => {
                        if (v.type.code === 'mysql') {
                            ...
                            var roleRunningCode = jsonJudgeNotDefined(v, "v.replication.slaveRunning.code");
                            var roleRunningDisplay = jsonJudgeNotDefined(v, "v.replication.slaveRunning.display");
                            var roleIoRunningCode = jsonJudgeNotDefined(v, "v.replication.slaveIoRunning.code");
                            var roleIoRunningDisplay = jsonJudgeNotDefined(v, "v.replication.slaveIoRunning.display");
                            var roleSqlRunningCode = jsonJudgeNotDefined(v, "v.replication.slaveSqlRunning.code");
                            ...
                            json.roleIoRunningCode = roleIoRunningCode;
                            json.roleIoRunningDisplay = roleIoRunningDisplay;
                            json.roleSqlRunningCode = roleSqlRunningCode;
        ...
        btnClick: function (code, array, getCheckboxTablerId) {
            ...
            switch (code) {
                ...
                case this.btnReplaceList.rebuild://重建
                    ...
                    height = '430px'
                    title = "重建"
                    url = "/" + getProjectName() + "/app/servgroup/mysqls/manageTab/mysql/rebuildMysql"
                ...
                case this.btnReplaceList.rebuildInit://重建初始化
                    ...
                    urlData = "?unitId=" + getCheckboxTablerId.id + "&source=mysql" + "&servGroupId=" + rowId
                    _this.selectedData = getCheckboxTablerId
                    parent.layerOpenFun(width, height, title, url + urlData)
                    break;
                ...
                case this.btnReplaceList.updateRole://角色设置//设置主
                    ...
                    height = '200px'
                    title = "角色设置"
                    url = "/" + getProjectName() + "/app/servgroup/mysqls/manageTab/mysql/updateRole"
        ...
        searchClick: function (settings) {
            let keyArray = ['relateId', 'ip', 'versionValue', 'cpuCnt', 'memSize', 'dataSize', 'logSize', 'host.ip', 'host.cluster.name',
                'roleRunningDisplay', 'roleName', 'archMode', 'statusText', 'podState.display', 'actionDisplay']
