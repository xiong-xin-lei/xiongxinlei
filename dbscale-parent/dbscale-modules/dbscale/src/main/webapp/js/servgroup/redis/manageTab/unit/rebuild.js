...
var typeRefres = getQueryVariable("typeRefres")
var archModes = getQueryVariable("archModes")
...
new Vue({
    ...
    methods: {
        ...
        formSubmit: function (formName, tableName) {//scaleUpModal
            ...
            let typeName
            if (archModes === 'single') {
                parentApp = parent[0].redisListApp
                typeName = "数据库"
            } else {
                parentApp = parent[0].unitListApp
                typeName = parentApp.typeName
            }
            let tempText1 = "对选中的" + typeName + "单元进行重建，重建操作会将原来的单元删除并创建一个一样的单元，"
            let tempText2 = ''
            let tempText3 = '请谨慎操作！'
            ...
            commonConfirm(parent.manageApp, '重建确认', getRiskLevelHtml(4, riskText) + getHintText('重建'), setting).then(() => {
                ...
                sendPut("/" + getProjectName() + "/units/" + unitId + "/rebuild", function (response) {
                    ...
                    parentApp.returnList(typeRefres)
                    _this.formClose()
                }, function (error) {
                    console.log(error)
                    operationCompletion(_this, error.response.data.msg, "error")
