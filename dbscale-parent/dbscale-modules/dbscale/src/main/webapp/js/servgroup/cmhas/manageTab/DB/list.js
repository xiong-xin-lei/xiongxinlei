var DBListApp = new Vue({
    ...
    methods: {
        ...
        addAjaxFun: function (getCheckboxTablerId) {
            ...
            var height = '265px'
            var title = "新增"
            var url = "/" + getProjectName() + "/app/servgroup/cmhas/manageTab/DB/add"
            var urlData = ""
            parent.layerOpenFun(width, height, title, url + urlData)
        },
        deleteAjaxFun: function (getCheckboxTablerId, successFun, falseFun) {
            var _this = this
            sendDelete("/" + getProjectName() + "/serv_groups/cmha/" + rowId + "/db/schemas/" + getCheckboxTablerId.name, function (response) {
        ...
        refreshAjaxFun: function (getCheckboxTablerId, successFun, falseFun) {
            var _this = this
            sendGet("/" + getProjectName() + "/serv_groups/cmha/" + rowId + "/db/schemas", function (response) {
                successFun(response, getCheckboxTablerId)
        ...
        btnClick: function (code, array, getCheckboxTablerId) {
            ...
            switch (code) {
                ...
                case this.btnReplaceList.table://表信息
                    ...
                    url = "/app/servgroup/cmhas/manageTab/DB/table?rowId=" + rowId + "&schemaName=" + getCheckboxTablerId.name + "&menu=" + encodeURIComponent(JSON.stringify(menuData), "utf-8")
                    json = {
                        code: url,
                        name: "表信息"
                    }
