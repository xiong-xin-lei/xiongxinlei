...
var tabParamCfgListApp = new Vue({
    ...
    created: function () {
        ...
        setTimeout(function () {
            getHtmlMinWidth("tabParamCfgList", _this, ['xTable'])
        }, 0)
    },
    methods: {
        dataDispose: function (data) {
            ...
            XEUtils.arrayEach(data.data, (v, i) => {
                ...
                var cansetText = ""
                if (v.canSet !== null)
                    cansetText = v.canSet ? "是" : "否"

                var mustRestartText = ""
                if (v.mustRestart !== null)
                    mustRestartText = v.mustRestart ? "是" : "否"
        ...
        editClosedEvent: function ({row}) {
            ...
            if (!reg.test(row.value)) {
                ...
                operationCompletion(_this, "值不能含有中文字符", "error", config)
            }
            ...
            if (flag) {
                commonConfirm(parent.manageApp, '修改确认', '是否将"' + row.key + '"的值修改为' + row.value + '?').then(() => {
                    sendPut("/" + getProjectName() + "/serv_groups/" + rowId + "/cfgs?type=" + _this.unitValue, function (response) {
                        ...
                        operationCompletion(_this, "参数配置编辑成功！", "success", config)
                        _this.returnList()
                    }, function (error) {
