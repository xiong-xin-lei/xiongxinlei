...
var scaleUpApp = new Vue({
    ...
    created: function () {
        this.highAvailable = rowData.highAvailable
        this.formData = {
            id: rowData.id,
            name: rowData.name,
            type: this.typeList[0].code,
            dataSize: rowData.dataSize,
            logSize: rowData.logSize
        }
    ...
    methods: {
        createdDataView: function () {
            var _this = this
            sendGet("/" + getProjectName() + "/serv_groups/redis/" + rowData.id, function (response) {
                var json = {}
