...
var cmhaListApp = new Vue({
    ...
    created: function () {
        ...
        setTimeout(function () {
            ...
            getHtmlMinWidth("cmhaList", _this, tableNameArray)
        }, 0)
    },
    methods: {
        ...
        isAble: function (code, data, type) {
            ...
            if (XEUtils.isArray(data)) {
                ...
                let taskActionCode = jsonJudgeNotDefined(data, 'data.task.action.code')
                let taskStateCode = jsonJudgeNotDefined(data, 'data.task.state.code')
                switch (code) {
                    ...
                    case _this.btnReplaceList.slowLog:
                    case _this.btnReplaceList.monitor:
                    case _this.btnReplaceList.monitorService:
                    case _this.btnReplaceList.updateRole:
                		return taskActionCode == 'force_rebuild' && (taskStateCode == 'failed' || taskStateCode == 'running')
                    case _this.btnReplaceList.rebuildInit:
                        return false
        ...
        usersCheck: function (row, openType) {
            ...
            var urlData = "?row_id=" + row.id + "&row_relateId=" + row.relateId + "&obj_type=cmha" + "&open_type=" + openType
            parent.layerOpenFun(width, height, title, url + urlData)
        },
        ...
        refreshAjaxFun: function (getCheckboxTablerId, successFun, falseFun) {
            var _this = this
            sendGet("/" + getProjectName() + "/serv_groups/cmha/" + rowId, function (response) {
                successFun(response, getCheckboxTablerId);
                layer.closeAll('loading');
            }, function (error) {
                falseFun(error, getCheckboxTablerId)
        ...
        btnClick: function (code, array, getCheckboxTablerId, type) {
        	...
        	console.log(code, array, getCheckboxTablerId, type)
            var _this = this
            ...
            switch (code) {
                ...
                case this.btnReplaceList.rebuild://重建
                    ...
                    height = "222px"
                    title = "重建"
                    url = "/" + getProjectName() + "/app/servgroup/cmhas/manageTab/unit/rebuild"
                    if (type === "mysql") {
                        url += "Mysql"
                    	width = "780px"
                        height = "430px"
                ...
                case this.btnReplaceList.rebuildInit://重建初始化
                    if (type === "mysql") {
                        ...
                        height = '453px'
                        title = "重建初始化"
                        url = "/" + getProjectName() + "/app/servgroup/cmhas/manageTab/unit/rebuildInit"
                ...
                case this.btnReplaceList.backup://备份
                    ...
                    height = "315px"
                    title = "备份"
                    url = "/" + getProjectName() + "/app/servgroup/cmhas/manageTab/unit/backUp"
                ...
                case this.btnReplaceList.restore://还原
                    ...
                    height = "355px"
                    title = "还原"
                    url = "/" + getProjectName() + "/app/servgroup/cmhas/manageTab/unit/restore"
...
window.onresize = function () {
    cmhaListApp.table_heighth = commonOnresize()
}