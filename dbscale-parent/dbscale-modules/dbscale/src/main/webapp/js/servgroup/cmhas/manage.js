...
var manageApp = new Vue({
        ...
        tabUrl: function () {
            ...
            switch (this.activeName) {
                case this.btnReplaceList.topology://部署架构
                    return "/" + getProjectName() + "/app/servgroup/cmhas/manageTab/topology?rowId=" + rowId + "&id=" + this.id
                case this.btnReplaceList.unit://数据库
                    return "/" + getProjectName() + "/app/servgroup/cmhas/manageTab/unit?rowId=" + rowId + "&code=" + this.btnReplaceList.redis + "&id=" + this.id
                case this.btnReplaceList.paramCfg://参数配置
                    return "/" + getProjectName() + "/app/servgroup/cmhas/manageTab/paramCfg?rowId=" + rowId + "&id=" + this.id
                case this.btnReplaceList.DB://库管理
                    ...
                    return "/" + getProjectName() + "/app/servgroup/cmhas/manageTab/DB?rowId=" + rowId + "&id=" + this.id + "&menu=" + encodeURIComponent(JSON.stringify(menuData), "utf-8")
                case this.btnReplaceList.user://用户管理
                    return "/" + getProjectName() + "/app/servgroup/cmhas/manageTab/user?rowId=" + rowId + "&id=" + this.id
                case this.btnReplaceList.backupStrategy://备份策略
                    return "/" + getProjectName() + "/app/servgroup/cmhas/manageTab/backupStrategy?rowId=" + rowId + "&id=" + this.id
                case this.btnReplaceList.backupFile://备份文件
                    return "/" + getProjectName() + "/app/servgroup/cmhas/manageTab/backupFile?rowId=" + rowId + "&id=" + this.id
            }
        }
    },
    created: function () {
    ...
    methods: {
        ...
        createdView: function (val) {
            var _this = this
            sendGetNoLoading("/" + getProjectName() + "/serv_groups/cmha/" + rowId + "?replication=false&topology=false", function (response) {
