...
var userListApp = new Vue({
    ...
    methods: {
        ...
        resetPwdAjaxFun: function (getCheckboxTablerId) {
            ...
            let width = '424px'
            let height = '265px'
            let url = "/" + getProjectName() + "/app/servgroup/mysqls/manageTab/user/resetPwd"
        ...
        pwdAjaxFun: function (getCheckboxTablerId, successFun, falseFun) {
            var _this = this
            let url = "/" + getProjectName() + "/serv_groups/mysql/" + rowId + "/users/" + getCheckboxTablerId.username + "/pwd/reset"
        ...
        deleteAjaxFun: function (getCheckboxTablerId, successFun, falseFun) {
            var _this = this
            let url = "/" + getProjectName() + "/serv_groups/mysql/" + rowId + "/users/" + getCheckboxTablerId.username
            if (!_this.highAvailable)
                url += "?ip=" + encodeURIComponent(getCheckboxTablerId.whiteIp, "utf-8")
        ...
        refreshAjaxFun: function (getCheckboxTablerId, successFun, falseFun) {
            var _this = this
            sendGet("/" + getProjectName() + "/serv_groups/mysql/" + rowId + "/users", function (response) {
                successFun(response, getCheckboxTablerId)
        ...
        btnClick: function (code, array, getCheckboxTablerIdData) {
            ...
            switch (code) {
                case this.btnReplaceList.add://新增
                    width = 688
                    height = 430
                    url = "/" + getProjectName() + "/app/servgroup/mysqls/manageTab/user/add"
                    _this.addAjaxFun(width + 'px', height + 'px', url)
                    break;

                case this.btnReplaceList.update://编辑
                    url = "/" + getProjectName() + "/app/servgroup/mysqls/manageTab/user/update"
